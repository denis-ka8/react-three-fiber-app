...
export function Room({ dimensions }: RoomProps) {
    ...
    const wallColor = new THREE.Color(0xc7ff73)
    const wallRoughness = 0.85
    const wallMetalness = 0.0

    const wallMaterial = <meshStandardMaterial color={wallColor} roughness={wallRoughness} metalness={wallMetalness} />
    ...
    return (
        <group>
            ...
            {/* Правая стена */}
            <mesh position={[length / 2, height / 2, 0]} rotation={[0, (Math.PI / 2) * 3, 0]}>
                <planeGeometry args={[width, height]} />
                {wallMaterial}
            </mesh>
        </group>
